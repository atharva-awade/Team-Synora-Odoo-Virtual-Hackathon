...
type Data = { stats: any; compliance: any };

export function DashboardView({ userName, initial }: { userName: string; initial: Data }) {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {s.recentTrips.length === 0 && (
                  <tr><td colSpan={5} className="py-8 text-center text-muted">No trips yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <Card className="glass p-5">
            ...
            {statusBars.map((b) => (
              <div key={b.label}>
                ...
                <div className="h-2 overflow-hidden rounded-full bg-surface-2">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${(b.value / maxBar) * 100}%`, background: b.color }} />
                </div>
              </div>
            ))}
          </div>
          <div className="mt-5 border-t border-line pt-4 text-xs text-muted">
            ...
            <span className="font-medium text-ink">
              {formatKm(s.recentTrips.reduce((a: number, t: any) => a + (t.plannedDistanceKm || 0), 0))}
            </span>
          </div>
        </Card>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="glass p-5">
          <h2 className="mb-3 flex items-center gap-2 font-semibold">
            <ShieldAlert className="h-4 w-4 text-amber-500 dark:text-amber-400" /> Licence Compliance Radar
          </h2>
          <div className="space-y-2">
            {comp.licenceAlerts.length === 0 && <div className="text-sm text-muted">All driver licences are valid for 60+ days.</div>}
            {comp.licenceAlerts.map((a: any) => (
              <div key={a.id} className="flex items-center justify-between rounded-lg border border-line bg-surface-2/50 px-3 py-2 text-sm">
                ...
                <span className={a.days < 0 ? "text-rose-600 dark:text-rose-300" : a.days <= 30 ? "text-amber-600 dark:text-amber-300" : "text-muted"}>
          ...
          <div className="space-y-2">
            ...
            {comp.serviceAlerts.map((v: any) => (
              <div key={v.id} className="flex items-center justify-between rounded-lg border border-line bg-surface-2/50 px-3 py-2 text-sm">
                ...
                <span className={v.remaining <= 0 ? "text-rose-600 dark:text-rose-300" : v.remaining <= 1000 ? "text-amber-600 dark:text-amber-300" : "text-muted"}>
